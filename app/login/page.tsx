...
export default function Login() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Basic validation
    if (!formData.matricNo.trim() || !formData.password.trim()) {
      setError('Please fill in all fields');
      setLoading(false);
      return;
    }

    const result = await login(formData.matricNo, formData.password);
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <h2 className="mt-2 text-lg sm:text-xl font-semibold text-slate-700">Welcome Back</h2>
        ...
        <div className="text-center">
          <p className="text-xs text-slate-500 mt-4">
            Secure login powered by UniportBuddy Student Portal
          </p>
        </div>
      </div>
    </div>
